...
2. **Operations Module** ([src/doctk/operations.py](../src/doctk/operations.py) - 242 lines)
...
2. **POC Summary** ([docs/POC-SUMMARY.md](POC-SUMMARY.md) - 212 lines)
   - Validation of POC
   - What works well
   - Usage examples
   - Next steps

3. **Specification** ([docs/SPECIFICATION.md](SPECIFICATION.md) - 1,468 lines)
...
4. **README** ([README.md](../README.md) - 153 lines)
   - Project overview
   - Quick start guide
   - Python API examples
   - Project status
...
5. **Contributing Guide** ([CONTRIBUTING.md](../CONTRIBUTING.md) - 200+ lines)
   - Development setup
   - Workflow guidelines
   - Coding standards
...
2. **Strategy Pattern** - Format adapters
3. **Pipe Operator** - Composable transformations
4. **Builder Pattern** - Document construction (planned)
...
2. **Pipe operator** - Intuitive, LSP-friendly syntax
3. **POC-first approach** - Validated design before over-engineering
4. **Comprehensive documentation** - Clear vision for future development
5. **uv for package management** - Fast, modern tooling

### Design Decisions

1. **Python first, Rust later** - Rapid prototyping over premature optimization
2. **Format adapters** - Clean separation of concerns
3. **Immutable by default** - Functional programming principles
4. **LSP from day one** - Thinking about IDE support early

### Technical Debt (Intentional)

1. **Limited node types** - Focusing on core patterns first
2. **Basic selection** - Advanced selectors in v0.3
3. **Single format** - Markdown only for POC
4. **No streaming** - Will add in v0.8 for large documents
